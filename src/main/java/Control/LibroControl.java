...
public class LibroControl {
    ...
    private Libro libro;
    public ListadoLibro libroNuevo;
...
**/
    ...
    public List<SelectItem> getSelectLibroOcupado() {
        this.selectLibroOcupado = new ArrayList<SelectItem>();
        LibroDao cated = new LibroDao();
        List<Libro> ls = cated.listarLibrosOcupados();
        selectLibroOcupado.clear();
        for (Libro opcion : ls) {
            SelectItem Item = new SelectItem(opcion.getIdLibro(),
                    opcion.getTitulo());
            this.selectLibroOcupado.add(Item);
        }
